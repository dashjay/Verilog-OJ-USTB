...
//        'email_verified_at' => 'datetime',
  ...
  public function user_batch_add($username, $name, $password, $batch)
  {
    //使用自带的哈希工具对密码进行hash
    $hashed_password = Hash::make($password);
    ...
    $b = new Batch();
    $batch_exists = $b->where('batch_name', $batch)->first();
    if (!$batch_exists) {
      $b->batch_name = $batch;
      if (!$b->save()) {
        return make_re()->error('batch create fail');
      }
      $this->batch_id = $b->id;
    } else {
      $this->batch_id = $batch_exists->id;
    }

    if (!$this->save()) {
      return make_re()->error('user create fail');
    } else {
      return make_re()->info('user create successfully');
    }
